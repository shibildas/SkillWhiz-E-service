...
module.exports.getJob = async (req, res) => {
  try {
    ...
    const reviews = await reviewmodel
      .find({ jobId: job._id, reviewModel: "user" })
      .populate("reviewBy", "username image")
      .limit(10);
...
module.exports.redeemVoucher = async (req, res) => {
  try {
    ...
    const updatedUser = await usermodel
      .findOneAndUpdate(
        { _id: userId, vouchers: { $ne: id } },
        { $addToSet: { vouchers: id }, $inc: { loyality: -points } },
        { new: true }
      )
      .populate({ path: "vouchers", select: "-users" })
      .select("-password");
