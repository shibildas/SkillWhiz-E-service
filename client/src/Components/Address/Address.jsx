      ...
      <div className="modal">
        <div className="modal-box relative">
          ...
          <div className="border-gray-400 border-2 p-2 rounded-2xl">
            {address.length != 0 ? (
              address.map((ele, index) => {
                return (
                  <div
                    key={index + 20}
                    className={`border border-zinc-400 rounded-xl font-semibold p-2 m-2 ${
                      select?._id === ele?._id ? "bg-cyan-200" : "bg-indigo-100"
                    }`}
                  ...
                  >
                    ...
                    <h1 className="">
                      {ele?.street}, PinCode: {ele?.pincode}
                    </h1>
                  </div>
                );
              })
            ) : (
              <div className="border border-zinc-400 rounded-xl font-semibold p-2 m-2 bg-indigo-100 h-24 flex justify-center items-center text-3xl">
                <label className="link" htmlFor="addAddress">
                  + Add a New Address
                </label>
              </div>
            )}
            ...
            <div className="m-2">
              <label className="btn btn-success" htmlFor="addAddress">
                Add New
              </label>
            </div>
          </div>
